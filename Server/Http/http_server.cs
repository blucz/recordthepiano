...
namespace Http {
        ...
        internal class _Connection : IDisposable {
            ...
            void _BeginWrite() {
                ...
                try {
                    ...
                    _socket.BeginSend(_writebytes, _writeoff, _writecount, SocketFlags.None, ar => {
                        lock (_server._lock) {
                            if (_isdisposed) return;
                            try {
            ...
            void ev_writedone() {
                if (_state == _State.Failure) {
                    ...
                    // clear out all state
                    if (_tx != null) {
                        _tx.OnCompleted(new HttpTransactionStats() { 
                            BytesReceived = _bytesin,
                            BytesSent     = _bytesout,
                        });
                        try { _tx.Request.Dispose(); } catch { }
                    }
                    _tx                             = null;
                    _readoff                        = 0;
                    _readcount                      = 0;
            ...
            void ev_headersdone() {
                ...
                var response = new HttpResponse(this, meth, gzip_permitted, _server.GZipContent);
            ...
            void ev_requestbodyready() {
                ...
                if (contentlength == 0) {
                    ...
                    if (_tx.Request.Method == HttpMethod.Post && 
                        ...
                        _requestbody_multipart = new HttpMultiPartFormDataHandler(_server.TempDirectory, boundary, _tx.Request.Headers.ContentEncoding);
    ...
    public sealed partial class HttpTransaction {
        ...
        internal void OnCompleted(HttpTransactionStats stats) {
            //Console.WriteLine("COMPLETED {0} in {1} out", stats.BytesReceived, stats.BytesSent);
            var evh = Completed;
            if (evh != null)
                _server._DispatchCallback(() => evh(this, stats));
        }
    ...
    public sealed partial class HttpResponse {
        ...
        readonly bool                           _gzip_permitted;
        internal long                           _total_bytes_sent;

        internal HttpResponse(HttpServer._Connection connection, HttpMethod method, bool gzip_permitted, bool defaultgzipcontent) {
            _connection   = connection;
            _method       = method;
            _gzip_permitted = gzip_permitted;
            GZipContent = defaultgzipcontent;
        ...
        void _Respond(HttpStatusCode status, Stream body, long tocopy) {
            // XXX: gzip partial gets?
            __Respond(status, body, tocopy);
        }
        void _Respond(HttpStatusCode status, Stream body) {
            if (_gzip_permitted && GZipContent) {
                Headers.SetHeader("Content-Encoding", "gzip");
                var gzipped = new MemoryStream();
                using (var gz = new GZipStream(gzipped, CompressionMode.Compress, true))
                    body.CopyTo(gz);
                gzipped.Position = 0;
                __Respond(status, gzipped);
            }
            else {
                __Respond(status, body);
            }
        }
        void __Respond(HttpStatusCode status, Stream body, long? maxtocopy = null) {
            long remaining_bytes = body.Length - body.Position;
            if (maxtocopy > remaining_bytes) maxtocopy = remaining_bytes;
            long length = maxtocopy ?? remaining_bytes;
            //Console.WriteLine("responding with {0} bytes from stream of {1} bytes", length, body.Length);
            ...
            BeginResponse(HttpStatusCode.OK, 
                          ev_buffer: responsebuf => {
                              ...
                              var read = body.Read(buf, 0, (int)Math.Min(left, buf.Length));
                              //Console.WriteLine("    copied {0} bytes", read);
                              responsebuf.Count  = read;
                              left -= read;
                              if (left == 0) 
