...
namespace Sooloos
{
    ...
    public class JSON
    {
        ...
        public static string JsonEncode(object json, bool pretty = false)
        {
            StringBuilder builder = new StringBuilder(BUILDER_CAPACITY);
            bool success = SerializeValue(json, builder, pretty, 0);
        ...
        protected static bool SerializeObject(Hashtable anObject, StringBuilder builder, bool pretty, int level)
        {
            builder.Append("{");
            ...
            while (e.MoveNext()) {
                ...
                if (pretty) _Indent(builder, level+1);

                SerializeString(key, builder);
                builder.Append(":");
                if (pretty) builder.Append(' ');
                if (!SerializeValue(value, builder, pretty, level+1)) {
                    return false;
                }
            ...
            if (pretty) _Indent(builder, level);
            builder.Append("}");
            return true;
        }

        static void _Indent(StringBuilder builder, int level) {
            builder.Append("\n" + new string(' ', level*4));
        }

        protected static bool SerializeArray(ArrayList anArray, StringBuilder builder, bool pretty, int level)
        {
            builder.Append("[");
        ...
        protected static bool SerializeValue(object value, StringBuilder builder, bool pretty, int level)
        {
            if (value is string) {
                SerializeString((string)value, builder);
            } else if (value is Hashtable) {
                SerializeObject((Hashtable)value, builder, pretty, level);
            } else if (value is ArrayList) {
                SerializeArray((ArrayList)value, builder, pretty, level);
